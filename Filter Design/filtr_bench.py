...
from ceoff_compute import FilterDesign
...
oscPlot.addItem(crosshair_v, ignoreBounds=True)
oscPlot.addItem(crosshair_h, ignoreBounds=True)
...
CursorLabelX = pg.QtGui.QLabel('X:')
genPageLayout.addWidget(CursorLabelX,1,inRowCount)
inRowCount+=1
CursorLabelY = pg.QtGui.QLabel('Y:')
genPageLayout.addWidget(CursorLabelY,1,inRowCount)
inRowCount+=1
...
def mouseMoved(e):
        pos = e[0]
        if oscPlot.sceneBoundingRect().contains(pos):
            mousePoint = oscPlot.getPlotItem().vb.mapSceneToView(pos)
            CursorLabelX.setText("X: %.2f"  %(1000*mousePoint.x()))
            CursorLabelY.setText("Y: %.2f"  %mousePoint.y())
            crosshair_v.setPos(mousePoint.x())
            crosshair_h.setPos(mousePoint.y())

proxy = pg.SignalProxy(oscPlot.scene().sigMouseMoved, rateLimit=60, slot=mouseMoved)
generator = Gen()
oscilloscope = Osc()
design = FilterDesign()

outputChannel = oscilloscope.addChannel("Output Channel", pg.mkPen('r', width=2))
